...
const op_recall_page_repaint: OpHandler = (c) => {
  ...
  // ---- `0x82` 的同一道动作（:1235-1244）：覆写色**钉进该窗快照** ----
  //   本 handler 上面那一步（`setPageText` 清快照 + `captureFontStyle`）已经把覆写色收进去了
  //   （所以这一行在这里是**幂等**的）；与 `0x82` 用同一个入口是为了让两侧的"覆写色进快照"
  //   只有一份写法（`0x82` 不清快照 ⇒ 它必须走只换颜色那一支，见 `applyOverrideColorToSnapshot`）。
  if (override) applyOverrideColorToSnapshot(e, win, hex6(bgrToRgb(fill)), hex6(bgrToRgb(outline)));
