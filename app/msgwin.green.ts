 ...
 *
 * ★**发布前就得还原**：`emitWin` 的**第一步**（`styleOfWin` 取色）与**第二步**（`msgWinSync` 回调）
 *   之间必须夹着还原 —— 引擎这一笔不留任何持久状态，而探针（`config1Chain` 的 `restyleByI082`）
 *   正是在 `msgWinSync` 回调里读 `slot.fontStyle` 来判断"这一笔到底走了哪条取色路径"
 *   （`snapshot: false` = 实时色回退）⇒ 把临时快照一直留到回调之后会让那条实测判据失去意义。
 *   所以这里**只换样式、不动快照**：`emitWinWithLiveOverride` 先取样式、再还原、最后才发回调。
 ...
 */
function applyOverrideColor(e: Engine, i: number, fill: string, outline: string): FontStyleSnapshot {
  const base = globalFontSnapshot(e);
  return {
    ...base,
...
function emitWinWithColorOverride(e: Engine, win: number, itemId: number | undefined, fill: string, outline: string): void {
  ...
  const style = { ...styleOfWin(e, w, itemId), ...applyOverrideColor(e, w, fill, outline) };
 ...
 * 把覆写色钉进该窗快照**仅用于本次 `emitWin`**（发布后把快照还原），再把全局两格恢复成调用前的值**。
 *
