 ...
 *   float 值池 → `sub_408050(buf, 1024, "%lf", v)`；float 指针族(7/13) → `default:` 抛 `Command_Type_Exception`。
 *
...
test('★0x192 set-string：int 值池/指针/立即数都按引擎**转十进制**（不再回槽号）', async () => {
  ...
  assert.equal(e.globals.str.get(OUT), '42', '引擎 `_itoa_s(DEC(池值), …, 10)`');

  // 局部 int 值池
  e.frames[0]!.locals.int.set(7, enc(e.key, -7));
  await runOp(e, 0x192, [gStr(OUT), lInt(7)]);
  assert.equal(e.globals.str.get(OUT), '-7', '负数带符号（`_itoa_s` 有符号）');

  // 立即数
  await runOp(e, 0x192, [gStr(OUT), immInt(12345)]);
  assert.equal(e.globals.str.get(OUT), '12345');
  ...
  assert.equal(e.globals.str.get(OUT), '99', '指针族 = `_itoa_s(DEC(*指针), …, 10)`');
});
...
test('★0x193 concat：数值操作数转串后拼接（复刻 src/COMMITDR.txt:9 的形态）', async () => {
  ...
  assert.equal(e.globals.str.get(OUT), '菲亚的女神等级提升为3', '★修前这里会得到「…提升为26510」（槽号）');
});

test('★0x1B2 text-append：按值的十进制追加（复刻 src/SYSTEM4.txt:464 的 `(global-int 0)`）', async () => {
  const e = engine();
...
test('★float 族按 C 的 `%lf`（默认精度 6）；float 指针族照引擎抛错', async () => {
  const e = engine();
  e.globals.float.set(11, 3.14);
  await runOp(e, 0x192, [gStr(OUT), gFloat(11)]);
  assert.equal(e.globals.str.get(OUT), '3.140000', '`sub_408050(…, "%lf", v)` 的默认精度 6');

  await runOp(e, 0x192, [gStr(OUT), immFloat(0x40490fdb)]);
  assert.equal(e.globals.str.get(OUT), '3.141593', '立即 float 走位模式 → `%lf`（0x40490fdb = 3.1415927f）');

  await assert.rejects(
    () => runOp(e, 0x192, [gStr(OUT), gFptr(1)]),
    /float 指针族/,
    '引擎 `sub_41B640`/`sub_42A420` 对 tag 7/13 走 default ⇒ 抛 Command_Type_Exception',
  );
});
