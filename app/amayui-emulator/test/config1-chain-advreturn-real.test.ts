...
test('★T-0102 判据 3 收尾：真路径实测值（g0=6 + 旁白 + 记录表非空）⇒ 门开、色转白、i082 用实时白重画**一次**', async () => {
  ...
  //   `T-0170` 把旧断言 `=== 3` 改成 `=== 6`，理由是"`0x6E`/`0x196` 现在也 push 已画行记录
  //   ⇒ 3 种子 + 3 行正文"。那个理由**缺了一道门**：`CONFIG.txt:41` 是 `i1bb 0`（⇒
  //   `Engine[97055] = 0x80000000`）、`:355` 才是 `i1bb 1`，而本链路的 ADV 样例正文在 `:177-179`
  //   —— **夹在中间**。引擎**每一处**记录 push 都查这道门（`sub_46BE30` raw 83941-83942 /
  //   `sub_4691D0` raw 81549 / `sub_46B100` raw 82726 / `sub_461A10` raw 76380-76381 的
  //   `v61 = a4 & 0x80000000` / 另一处 raw 83026 / 83307 / 83634），`0x6E`/`0x196` 把
  //   `_this[97055]` 当第 5 实参传下去（raw 28364/28375、29088/29104）⇒ **这条链上一条正文记录都不记**。
  //   ★可复现的判据实验（`.tmp/t0151-msgwin/probe-i1bb3.mts`）：`seedRecords: 0` 时
  //   `recordsAtI082 === 0`，而同一探针的 `sampleWin` 明确显示样例窗已排好（1 行 10 字 + 2 注音）
  //   ⇒ 抑制来自这道门、不是"文本没入队"。
  //   ★`T-0102` 的结论**不受影响**：真实路径是「ADV（`i1bb` 正常）→ CONFIG」，那些行在 ADV 阶段
  //   就已经记下 ⇒ 表非空 ⇒ `i082` 照样重画（见 `config1-chain-advreturn.test.ts` 的条件分支）。
  assert.ok(r.recordsAtI082 >= 3, '种子落进记录表且**非空**（门读的就是它；下界 = 种子数）');
  assert.equal(
    r.recordsAtI082,
    3,
    '★真值 = **只有 3 个种子**：本链路（TITLE → CONFIG）的正文入队发生在 `i1bb 0` 区间内 ⇒ 引擎不记（raw 83941-83942 / 81549）',
  );
