...
// ---------------------------------------------------------------------------
// ★bit16 的**门** = `sub_46BE30` 的返回值（= 本行有内容吗）：空行不得置位
// ---------------------------------------------------------------------------

test('op-3-004 ★空行：文本块内 `0x6E` 推空串 ⇒ 不置 bit16（`sub_46BE30` 的返回值门，raw 28330-28334）', async () => {
  const e = mkEngine([instr(0x304, []), instr(0x6e, [im(1), str('')])]);
  await stepOnce(e); // 0x304 → flags = 1
  assert.equal(e.msgwin.flags & 1, 1, '前提：文本块已开');
  await stepOnce(e); // 0x6E 空串
  assert.equal(
    e.msgwin.flags & 0x10000,
    0,
    '★raw 28330-28336：`result = sub_46BE30(...); if (result) { … |= 0x10000 }` —— `*a3 == 0` 时 `sub_46BE30` 提前 `return 0`（raw 83493-83497）⇒ **空串不置 bit16**',
  );
  assert.equal(e.msgwin.lastArg, 1, '入队支照旧写 `_this[122371] = op1`（raw 28335）');
});

test('op-3-004 ★空串注音：文本块内 `0x196` 空本文词 ⇒ 不置 bit16（同一道返回值门，raw 29104-29108）', async () => {
  const e = mkEngine([instr(0x304, []), instr(0x196, [im(1), str(''), str('あまゆ')])]);
  await stepOnce(e);
  assert.equal(e.msgwin.flags & 1, 1, '前提：文本块已开');
  await stepOnce(e);
  assert.equal(
    e.msgwin.flags & 0x10000,
    0,
    '★raw 29105：`if (result)` —— 本行文本为空 ⇒ 不置 bit16（`0x305` 的总门 `(flags & 0x10001) == 0x10001`，raw 26045，会因此只清 flags）',
  );
});
