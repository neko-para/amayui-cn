...
/**
 * ADV 位的置/清 —— **2026 修正**：`0x071` 不再无条件置位；**2026-09 再修正（`T-0151`）**：
 * `97050`（跳读/自动模式）**不是**"门关时也保留显示态"的依据。
 *
 ...
 * 门控（随包 SYS4REG.INI 里 = 0）。两条支**分开**（raw 28432-28460）：
 * ```c
 * 门**关闭**： if (!122455) goto LABEL_11; else 122455 = 0;      // ★97050 完全不参与
 * 门**打开**： if (!sub_48F000(...)) { if (97050) goto LABEL_11; else 122455 = 0; }   // "保持"
 *             else { 174801 |= 0x8000000; 122455 = 1; }          // 置位
 * ```
 * ⇒ 本用例原先在"门关 + 97050=1"下断言 `advActive === true`，那是**修前缺陷**（把"保持"支接错了路）；
 * 现在按体改成"门关 ⇒ 不置"。`0x19C` 那一半不变（`op_adv_enter` 自己的门就是 `97050`）。
 *
...
test('advActive(0x8000000)：由 0x088/0x19B/0x19C 置/清，且 0x071 不再无条件置位', () => {
  ...
  assert.equal(e.advActive, false);
  // ★门**关闭**时 97050 不参与：0x071 依旧不置 ADV（raw 28456-28460）
  OPS.get(0x071)!(step(0x071, [im(0), { type: 2, raw: 0, str: 'y' } as never]));
  assert.equal(
    e.advActive,
    false,
    '★门关 ⇒ `122455 = 0` 且不置 `0x8000000`（raw 28456-28460）—— 旧断言"跳读模式下置 ADV"编码的是修前缺陷',
  );
