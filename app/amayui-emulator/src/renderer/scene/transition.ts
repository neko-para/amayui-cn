 ...
 *
 * **② 源的决策（`T-0103` 轮 17 修订：源 = 区间 A 的离屏副本，不是"本帧屏幕"）**：
 * 引擎把 effect 的 `Tex0` 设成**层 36 的纹理**（raw 135883-135884，`Scene+42600` = 层表 42456 + 4×36），
 * 而层 36 装的是那两趟 item 重绘（raw 136014-136176）写进去的**区间项** ⇒ 被模糊的就是**区间项本身**。
 * 轮 15 曾据"类别 3 的体跳过那两趟填充（asm `0x4B3187: cmp eax,3 / jnz loc_4B379C`）+ 本帧刚清过层 36
 * （raw 135824-135825）"推出"模糊的是一张空/陈旧 scratch ⇒ 与真机矛盾 ⇒ 改取本帧屏幕"，**那条推理的结论是错的**：
 * 在 emulator 里"本帧屏幕"已经把区间 A 的项**排除出屏幕 pass**（D3：`scTransitionMarkedHandles` →
 * `presenter.present` 的 `skipped()`），而区间 A 恰恰就是被模糊的那块 ⇒ 取屏幕 = 模糊一张**没有区间项**的
 * 画面 = **整屏黑**（`tickets/T-0103` 轮 17 实测：SN0000 结尾的 2500ms 横向模糊整段看不见、画面从云柱背景
 * **直接切成黑**；同一窗口的 `[present]` 摘要里那块过渡项 α=202 正盖着屏幕 ⇒ 槽里就是黑的）。
 * ⇒ 决策：宿主按**区间 A 的离屏子集**（`#renderRangeCanvas(handles.a)`）当源 ——
 * 它就是"层 36"在本重写里的等价物（与类别 0/2 同一份机制、同一份已披露的"每帧重渲而非首帧冻结"近似）；
 * 区间 A 为空时才退回屏幕合成（引擎那时读到的是陈旧内容，属退化路径，宿主日志会写明）。
 */
