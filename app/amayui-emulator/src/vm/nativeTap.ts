...
const WHY: Record<string, string> = {
  ...
  l2dLoadModel: 'Live2D 装 .MOC：不实现 ⇒ 槽里没有模型 ⇒ 572B 节点整块不出画（引擎同门控，无报错）',
  l2dBindTexture: 'Live2D 装纹理：不实现 ⇒ 立绘无贴图（画不出或画成空白），无报错',
  l2dStartMotion: 'Live2D 装 .MTN：不实现 ⇒ 立绘定格在默认姿态（动作永不动），无报错',
