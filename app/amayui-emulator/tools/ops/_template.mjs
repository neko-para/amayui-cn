#!/usr/bin/env node
/**
 * ops/{{NAME}}.mjs —— **用例：{{PURPOSE}}**（{{DATE}} 由 `amayui_emulator action=op-create` 生成）
 *
 * ★**还没填的三样**（缺一不可，见下）：① 开屏手势 ② 机器可读判据 ③ 该场景特有的坑 ——
 *   填完之后去 `ops/README.md` 的表里加一行（工具 `action=ops` 读的就是那张表 + 各文件头）。
 *
 * ─────────────────────────────────────────────────────────────────────────
 * 以下是模板原文（`ops/_template.mjs`）：**新增一个"从某个界面做某事"用例**的说明。
