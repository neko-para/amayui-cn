...
> **说明**：本 §2 首批/子系统记录写于工程早期（当时 M0 未编码）。**当前 M0–M3 已达成**（解释器能跑到 TITLE，`npm test` 12/12（★M0–M3 时快照；2026-09 实测 380））；§2 的 `studying` 条目大多已实现或已确认，但**尚未逐条回填 `rewritten`**（按 §3 的「旧条目待 M1 回填」约定）。**本次会话新增的确认见 §2.3。**
...
| 0xB8 | B8 | `sub_419720`(24817) | **rewritten** | **停 BGM**（0 操作数）：清 `effect_flags` bit0x200（在则推进淡出 `sub_489E50(Music,100)`）→ `sub_489B50` 停播；**不动 `sound:Music`**。语料 `MMODE.txt:63/462/552`（BGM 鑑賞进界面/换曲先停上一首）；2026-09 转真实现（`handlers/audio.ts` → `{kind:'bgm-stop'}`） |
