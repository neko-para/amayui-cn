...
| [03-数据模型与数据管道.md](03-数据模型与数据管道.md) | 统一 schema、**TS 类型契约**、名称中文化、索引/规划图、一键提取 | 高 |
...
- **计数已核对**：`metadata.json` 实测 **items=923，buildings=113，recipes=458（物品 356 / 建筑 102），units=243（带掉落 203），dropEntries=648，去重掉落物品 252**——与主工程已知反推结果一致（旧文档「194 / 71」计数**过时**）。
