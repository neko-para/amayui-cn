...
另有一条方法学提醒（审计报告末尾）：任何「按构造期地址反解 opcode」的分析都要先把该地址在构造函数里核对到赋值行 —— 本次核对后确认 0xFB..0x10F 全都有赋值（先前一个 agent 提出的『偏移漂移』结论不成立），而 opcode-table.md 里 30 行 status 为空的行是**别的 AGE 作品**的交叉引用（Amayui 2 / Tenmei / Fuukan / Hyakusen），不是本引擎的表项，属于正常设计。
