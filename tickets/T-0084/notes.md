...
新文档 **`docs-new/99-records/2026-09-transition/transition-render-spec-2026-09.md`**（8 节：速览 / 24 格全表 / 消费端算法 / 平面层序 / emulator 集成点 / 步骤+守卫 / 未读到 / raw 锚点表）。`sub_4B06D0` 的体**已全覆盖读过**（134417-136739，分 7 段）。
...
★本票已按验收收口（2026-09 轮 6，状态 done）——现行状态以能力台账 `clock-read-transition-window`（modeled-verified/E3）与 `tickets/T-0084/notes.md` 为准：
① 记录表 `Scene+1048`（96 B / 24 dword）写入端：`0x223`（类别 0）/`0x24F`（类别 2）/`0x250`/`0x251`（类别 3）逐格照抄 `sub_4AAE10`，运行期值另存 `render4.transitionRuntime`；
② 窗口模型 `src/renderer/scene/transition.ts`（首帧锁存 `[1]`、按 clock 算 t、到点杀记录、到点后清空整表，raw 134867-134871 / 136840-136841）；
③ 12 种条带几何全按 raw 134947-135510；
④ 渲染端类别 0/2/3 都画进 `[4]` 的离屏槽，类别 0/2 的源 = 记录那两条 item 区间的子集（`presenter.renderItemSubset` + `pixiBackend.#renderRangeCanvas`），旧的「上一帧整屏快照」概念已删除。
★**仍未建模的四项已转 `T-0091`**（不许用「已 done」掩盖）：类别 3 的 `(2L+1)²` 核逐点权重未复刻（现为已披露近似）、`[4]` 指向非 `create-texture` 槽、`Scene+46508/46512/46516` 三标志本身未建、E4 像素级可达路径缺失。
★规格文档 = `docs-new/99-records/2026-09-transition/transition-render-spec-2026-09.md`；`U3/U4` 已由读 asm 收敛（36/37 = 两条 item 区间；反编译那段 `while(1)` 恢复是错的）。
