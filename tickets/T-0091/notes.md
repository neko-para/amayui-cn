...
按 `tickets/T-0103/evidence/chain-audit-instructions-capabilities.md` 的 D2/D3/D4，本票新增两条验收（到期帧合成 / 区间项屏幕排除），并把 D4（类别 3 的源=本帧屏幕、核=累积近似）与 U3 的关系记在上面。★E4 通道本轮已可用：`node app/amayui-emulator/tools/load-slot.mjs --slot 78` （一条命令到读档完成的 `[slot-load]` 判据）+ `capture`；**但先修 `T-0144`**（0x1F6/0x1F7 未清 572B 表），否则残留立绘会污染转场像素对照。
...
**④ E4 可达路径**：`app/amayui-emulator/tools/load-slot.mjs --slot 78` 让"读档到 SN0000 末页 → 下一步就是切章"变成一条命令（判据 = 日志 `[slot-load]`）。
