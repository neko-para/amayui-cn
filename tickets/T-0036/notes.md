...
## 第 3 轮（2026-09）：用户截图确认（E4）

用户提供的**模拟器存档列表截图**里，右侧每一行的缩略图都已正常显示（LOAD 110–115 六行各有小图，
116–119 空槽无图）——与真机排布一致。这是本票的**用户侧 E4 观感确认**，与代码侧证据
（`0x1AF` 把 `.STH` 的 BMP 解进 `op3` 纹理槽）互为印证。

同轮用户也确认第 1 条缺陷（字体偏粗）**没有变化** —— 那条与缩略图无关，属于 T-0035 的未收敛部分。
