...
## 收口复测（2026-09-23，T-0126 关单同轮）

| 口径 | 值 |
|---|---|
| `npm run verify` 全流程 | **34.6 s**（1086 例 / 1084 pass / 0 fail / 2 skip；含 4× tsc + `test:all` + 死写棘轮） |
| 其中 T0 默认档（`npm test`） | ~6 s（124 文件 / 797 例） |
| 闸门 D 组成变化 | 本轮起 `check:typecheck-test`（131 条基线棘轮）→ **`typecheck:test`（零容忍）**：基线归零后棘轮退役，见 `T-0126` 第 3 次变更 |

★与上表 `51.4 s → 31.3 s` 的差（31.3 → 34.6 s）来自**同轮并行负载**与用例数增长（1086 例 vs 1086 为同一批；T1 侧本轮无改动），
不是判据变化。三条候选路径里落地的是 **②分档**（T-0126）+ **③减少重复链路跑次**（`config1-chain` 拆 5 文件并行、两处按键 memo、`save-slot-chain` 拆两文件），
**①共享 fixture 未采用**（风险：状态泄漏 ⇒ 需要引擎态快照/回灌 = `T-0122` 的能力）。
