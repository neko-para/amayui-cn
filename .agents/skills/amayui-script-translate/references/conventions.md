...
3. 构建校验：
   - 在 `scripts` 目录运行 `npm run assemble -- <SCRIPT>`，必须通过
     （骨架校验/SJIS/回读验证；Node 版 age-asm 跨平台，任何平台均可运行）。
   - 若尚未构建 install 树 / 无法运行游戏验证：改用本地校验：`node scripts/reflow-apply.js --check <SCRIPT>`
     （三段式页块流程时）、`node scripts/find-untranslated.js <SCRIPT>`、宽度/结构检查
     （见 references/verify.md「本地校验」节）；完成后按下节格式写入项目根 `PENDING.md`。
...
5. **记录**：
   - 完成 assemble 后：更新 `PROGRESS.md`（把脚本加入已翻译内容索引）、
     `patch/patch.config.json`（把 assemble 产物 BIN 加入补丁同步清单 files）与
     `patch/CHANGELOG.md`（按「变更记录」节追加本次改动条目）。
   - 若只做了本地校验、尚未 assemble：只写项目根 `PENDING.md`（完全新翻译与修改均登记），不更新
     `PROGRESS.md` / `patch/patch.config.json`，等 assemble 通过后再按上面登记。
6. 不修改 `data\`；不执行 git 提交。

## 本地校验流程与 PENDING.md 登记

若尚未构建 install 树 / 无法运行游戏验证（如仓库在 macOS、游戏仅在 Windows 侧验证），可暂不 assemble
（Node 版 age-asm 本身跨平台，任何平台都可 `npm run assemble`，此时仅指未能跑通/未运行）。
文本更新完成后，在项目根 `PENDING.md`（不存在则新建，含顶部用途说明）登记「已翻译、未编译」条目，
供稍后能 assemble 的环境补做编译；条目按日期分节、每脚本一条：
...
- 待办：在 `scripts` 目录运行 `npm run assemble -- <SCRIPT>`，通过后登记
...
- `PENDING.md` 只含未编译条目：assemble 通过并登记 `PROGRESS.md` /
  `patch/patch.config.json` 后，删除对应条目，避免与 PROGRESS.md 重复记账。

## 变更记录（patch/CHANGELOG.md）

每次修改完成（assemble 通过并登记 `PROGRESS.md` / `patch.config.json`）后，
必须同步在 `patch/CHANGELOG.md` 追加一条变更记录；仅做本地校验未 assemble 时不写本文件
（改动已登记在项目根 `PENDING.md`，待 assemble 通过后一并追加）。

### 格式

- 版本节：当前开发版本写作 `## vX.Y（开发中）`（不带日期）；发布后补发布日期写作
  `## vX.Y（YYYY-MM-DD）`；新版本在前；
- 追加规则：新条目添加到最新一个标记「开发中」的版本节**最上方**（最新在前），
  不从下方追加；若不存在「开发中」节（如刚发布过），先新建
  `## vX.(Y+1)（开发中）` 再添加，不直接写进已发布节；
- 条目间**空一行分隔**：同一版本节内，相邻两条 `- [类型]…` 条目之间必须保留一个空行
  （即每条 bullet 独占一段，不与上下条目粘连），新增条目时同样按此格式插入；
...
- PROGRESS.md 与 patch/patch.config.json 更新情况（仅本地校验流程报告「待 assemble 补登记」）；
- 不确定/存疑名词清单。
