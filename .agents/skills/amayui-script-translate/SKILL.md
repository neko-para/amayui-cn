...
description: 直接执行《天結いキャッスルマイスター》汉化工程的游戏脚本翻译流程：把 src/*.txt（如 SC0000、SC0001、OPINIT1）从日文翻译为简体中文，并按既有约定完成排版（≤25 字/行）、构建校验（assemble）。当用户要求：翻译/重译某个游戏脚本、重排已翻译脚本、或并行翻译多个脚本时，使用本技能（本会话内直接执行，不借助 codex CLI 子进程）。
---
...
本会话内**直接执行**游戏脚本翻译流程，不调用 codex CLI 子进程：逐页翻译 `src\<脚本>.txt`，遵循工程既有约定（翻译语法、注音策略、块注释存档、≤25 字折行、术语表），最后用 `assemble` 校验。多个脚本可在本会话内依次处理（并行 = 每会话处理一个脚本，多开会话即可）。
...
3. **构建校验**：在 `scripts` 目录运行 `npm run assemble -- <脚本>`，必须通过
...
6. **记录**：翻译完成后更新 `PROGRESS.md`（把脚本加入已翻译索引）与 `patch/patch.config.json`
   （把 assemble 产物 BIN 加入补丁同步清单）。
7. **报告**：翻译统计、术语应用、assemble 结果、文档沉淀路径、存疑名词清单。
...
- 记录：翻译完成后改 PROGRESS.md 与 patch/patch.config.json。
- 不修改 data/；不执行 git 提交。
