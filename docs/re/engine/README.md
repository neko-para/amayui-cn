...
| 文档 | 主题 |
|---|---|
| [`00-脚本字节码格式与工具链.md`](./00-脚本字节码格式与工具链.md) | AGE 脚本字节码格式（SYS4/SYS4450 头、字符串编码）、反汇编/重汇编工具链（age-asm/unpack_alf）；指向 `06` 的权威 opcode 表 |
...
| [`04-引擎侧运行时数组.md`](./04-引擎侧运行时数组.md) | 引擎侧运行时数组（`0x53f48c`/`0x928a7` 等）在掉落、随机池中的作用（与 src/03 交叉引用） |
