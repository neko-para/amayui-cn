...
- 注意：`5D880/5D898`（.lst 里的帧结构）是消息泵/调用栈结构，**不是** bytecode 指针；bytecode 指针在 `[383128]`（120×cur_script + 383128）。
  - ★勘误：帧基址是 **0x5D880**（即 `120*cur + 383104`）；`383128` = 帧基址 **+0x18**（`ip`），`383124` = **+0x14**（`str_table`/脚本缓冲），`383220` = **+0x74**（操作数记数，主循环用它 `ip += 4*`）。`0x5D894`(=383124) 是 frame0 的 `str_table` 槽、**不是**帧基址——早期文档把它当帧基址，导致帧内偏移一律少 0x14。
