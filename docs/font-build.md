...
> ★**前置条件 / 实测反例（2026-09-15，`tickets/T-0035` 第 5 轮）**：上面这条「会真的变粗」**只在两面都被系统认到同一族时**成立。
> 用户真机（两份都装、完全退出重启游戏、5 个分类都设成 `Amayui CN`）实测：ADV 正文**没有变粗** ——
> 同一句台词按票里的口径（墨迹覆盖率）量出来是 **0.257**，落在本工程 Regular 档（独立复现 0.209-0.241）
> 而不是 Bold 档（0.345-0.379；两面差 1.46-1.59× ⇒ 该度量足以判定用的是哪一面）。
> ⇒ 真机上那次 `i2bd 1` 的 `lfWeight=700` **静默退回了常体**，最可能就是 §8.5.2 的**安装/缓存**问题
> （装完没重启 `FontCache` 服务/电脑，或 `%LOCALAPPDATA%\Microsoft\Windows\Fonts\` 里留着缺 932 码页声明的旧版
> ⇒ 游戏用 `lfCharSet=DEFAULT_CHARSET`（日文 locale ⇒ 932）选面时看不到 Bold 面）。
> 复核手段：游戏字体选择器里 `Amayui CN` 应只有**一条**；重装两份 + 重启 FontCache/电脑后 ADV 应明显变粗。
