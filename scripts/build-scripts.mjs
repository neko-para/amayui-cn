...
function indexPage() {
  ...
  L.push('## 怎么用（流程）');
  L.push('');
  L.push('分析某个 `src/*.txt` 时的动作顺序（流程与纪律详见 **`amayui-script-analysis`** 技能 §1/§2；引擎层见 `amayui-engine-analysis`）：');
  L.push('');
  L.push('0. ★**先读文档再读脚本**：`node .agents/skills/amayui-script-analysis/scripts/brief.js <ID>` 打印"开工前一页纸"');
  L.push('   （台账条目 + 文档落点 + 真源骨架 + 调用关系 + 收尾命令）；本页（`<ID>.md`）就是它的 ① 块渲染物，**先读它**；');
  L.push('1. `scripts.js --id <ID>` 看真源条目；没有就 `--add` 一个 `partial`/`stub` 骨架（`role`/`entry` 先写一句话），');
  L.push('   然后**只补 `notes` 里写的"未读"部分**，`layout` 已覆盖的段落不必重读；');
  L.push('2. 读脚本时**顺手记** `layout`（行区间 + 锚点 + 职责）与 `slots`（槽号 + 含义）——锚点用脚本里真实存在的字符串（label / opcode 行）；');
  L.push('3. 引擎层面的结论照旧进第一/第二层，并在 `links` 里回链（`capabilities` 填 id、`functions` 填 addr）——**分层不许互相复制**；');
  L.push('4. ★**改完必须同步更新**（不落库 = 没分析）：`node scripts/build-scripts.mjs` 重生成 md ⇒ `scripts.js --validate` ⇒ `npx tsx --test test/script-ledger.test.ts`；');
