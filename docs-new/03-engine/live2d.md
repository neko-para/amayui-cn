...
★**轮 6 订正与缺口**（`tickets/T-0077`/`T-0096`）：`0x347`–`0x34D` 七条已按体改对操作数类型/顺序/落点（含 `0x348` 由「缩放」改为**轴角旋转**、`scale` 扩三分量、补窗门 `record[0] & 1`，raw 134157/134202/134254）；真语料收益 = `i34d` **12 处（BTL）** 此前 delay/dur 与平移分量整体错位。★**仍缺**：节点矩阵合成器 `sub_4A07F0`（raw **121131-121520**，逐窗插值 + 颜色 + `D3DXMatrix*` 组合）未实现 ⇒ `L2dNode.scale/rotation/translate/baseOffset/wins` **有值而无消费端**（`l2dNodeTransform` 恒单位变换），已开 `T-0096`。
...
| ★节点的 `+508` 4×4 | `sub_49CA10` 初始化成**单位阵**，只有 `0x346`-`0x34D` 会改它；**本作语料只用 `0x344`** ⇒ 出画路径恒为单位阵 | raw 118508-118511（`+508/+528/+548/+568 = 1.0`）；raw 134385（`Scene+46532` "有节点矩阵"位） |
...
- 572B 节点的 4 组窗口（+28/+48、+32/+52、+36/+56、+40/+60）与颜色（+68）的插值推进细节
  ⇒ 也意味着 `0x346`-`0x34D` 的**矩阵组装顺序**未逐行确证（本作语料不触发这条路径）；
