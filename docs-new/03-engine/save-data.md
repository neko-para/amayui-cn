...
| 4 | 装载路径**不清绘制容器**：`sub_410160`（raw 19276-19936）全部 **27 个**被调函数里没有任何清容器调用（无 `sub_4AB7A0`/`sub_41A130`/`sub_40BF80`）。唯一的整批释放是 raw **19378-19389** 的两段**配置门**：① `CreateObject&2 && AutoFreeTexture&2 ⇒ for i<1000 sub_49E980(Scene,i)`（释放 1000 个**槽对象**）；② `CreateObject&2 ⇒ sub_40BFE0(+1064)/sub_4A9D10(+1096)/sub_4A1A60(Scene,j)`（清两个**网格**容器）。引擎默认 `set:CreateObject = 1`（raw 111733）/`set:AutoFreeTexture = 0`（raw 111742）⇒ 两段 `& 2` 都为假 ⇒ **都不执行**。装载路径里唯一的"屏幕级复位"是 raw **19913-19915** 的两个 **仮想ディスプレイ**复位 `sub_403EF0(Engine+51904 / +21976)`（对象族 `sub_403D70/403E70/404D E0/404E00/404E20/403AE0/403F40`）。⇒ 上一屏的绘制项在引擎里**照样留在容器里**；画不画得出来由**槽对象**决定（绘制期 `Scene[slot+10614]` 对象指针 + `Scene[5*slot+466]` 槽表），而 `i0ae`（`sub_4192F0`）只调 `sub_40F750` 重装帧、**不重建任何绘制项** | 单模型 + 每帧重组：上一屏（标题/读档菜单）的绘制项留在 `scene.drawItems` 里，而它们引用的槽刚被第 5 步换成新图 ⇒ 画出一屏拼贴（2026-09 用户实测：上半屏标题、下半屏序章、菜单压最上层）——★注意其中"下半屏 = 序章背景"正是引擎机制的**同一半**在起作用 | ✅→⚠️ **2026-09 起改为引擎口径**（`tickets/T-0083` 的 (B) 步）：装载点**不再**整批 `clearDrawContainer()`/`clearMeshSlots()`，改为只丢掉**被放弃的那条调用链**（caller 帧 + 它调用出来的帧；祖先帧不碰）画的绘制项（宿主缝 `native.dropFrameItems(frame)`，逐帧调用；`Item.ownerFrame` 记账）—— 引擎那两个 仮想ディスプレイ 复位（`sub_403EF0`）的语义就是"那一层 UI 整体不再组成"，而 emulator 是单一扁平表、没有"平面"对象 ⇒ 用"这一项是哪一帧画的"近似那一层；ADV 层（别的帧画的）保留。守卫 `test/slot-load-screen.test.ts`。★**实测残留（E4）**：`npm run shot -- --load 79` + 日志分桶 `剩余绘制项按归属帧分桶 = {-1:82, 1:18}` —— ① `1` = `TITLE.BIN`（菜单底下那层，句柄 10..308；引擎里它层序最低、被后画的序章背景盖住，而 emulator 这一路**压根没有序章背景项**，见 `T-0066`）；② `-1` = **不走 `0x1FB` 的建项路径**（文本项/`ensure` 建项）⇒ 归属还没记账。缺口 = 平面（离屏 render-to-target）合成模型 + **全建项路径的归属记账** |
