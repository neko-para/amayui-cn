...
| 0x400 等待门 | `sub_407E20` 12679（图形池 pending `_this[369348]`）★**2026-09 订正**：行号应为 **12762-12786**，且返回值 = **池挂起位 `_this[11629]`（= 字节 369348）叠加一个等待计时器**（起点 `_this[11630]`=369352、时长 `_this[11631]`=369356，配 `_this[11625]`=369332 毫秒时钟）。**计时器的装载者是 `0x238`**（`sub_4248C0` raw 32303-32312：`Engine[92338]=0; Engine[92339]=op1`）⇒ 脚本里 `i238 N` + `wait` 就是"等 N 毫秒"（见 `tickets/T-0024`、`opcode-table.md` 的 0x238 行） |
