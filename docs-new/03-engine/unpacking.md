...
| `engine/engine.cpp` | 由 `scripts/re/retarget.py` 从 `_utf8.c` 生成的成员化/语义化版（语义参考） |
| `scripts/re/` | 反汇编→重定型管线脚本 |
...
| 一键重定型 | `scripts/re/retarget.py` | 签名替换 + 字段标记 + 调用点 `this->` + 语义命名（`semantic_names.json`）→ `engine/engine.cpp` |
...
- ✅ 引擎是通用解释器、无业务语义；打包壳判定（ASProtect）、干净脱壳版可用；重定型管线可一键生成 `engine.cpp`。
- 🟡 成员函数识别边界（高偏移基准 + 双向传播，1239 个）需抽查复核。
