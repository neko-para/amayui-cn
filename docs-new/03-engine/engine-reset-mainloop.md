...
int sub_40DF10(int _this) {
  ...
  // ---- ④ 队列/栈容器重建（★实测：**构造点各 1 处**，各自在 do-while 里跑到计数 10 ⇒ **10 个 Queue_int + 10 个 Stack_int 对象**）----
  v18 = _this+388252; v15 = 10;                     // 18080-18081：10 dword = _this+388252..+388292   [平台无关/未建模]
  do {                                              // 18082-18109
    if (*v18) (**v18)(*v18, 1);                     // 18084-18085 释放旧的
    v6 = new(0x1C) Queue_int;                       // 18086/18091：对象 28B + Queue_int vftable
    v6[1] = new[](0x400);                           // 18092：内缓冲 1024B
    v6[4] = 256; v6[5] = 256;                       // 18093-18094：容量/上限各 256
    v6[2] = 0; v6[6] = 0; v6[3] = 0;                // 18095-18097
    *v18 = v6; v8 = v15-- == 1; v18 = v7 + 1;       // 18105-18107
  } while (!v8);                                    // 计数 10 次
  v18 = _this+388292; v16 = 10;                     // 18110-18111：10 dword = _this+388292..+388332   [平台无关/未建模]
  do {                                              // 18112-18137
    if (*v18) (**v18)(*v18, 1);                     // 18114-18115 释放旧的
    v10 = new(0x14) Stack_int;                      // 18116/18123：对象 20B + Stack_int vftable
    v10[1] = 256; v10[2] = 256;                     // 18121-18122
    v10[3] = new[](0x400);                          // 18124：内缓冲 1024B
    v10[4] = -1;                                    // 18125
    *v18 = v10; v8 = v16-- == 1; v18 = v11 + 1;      // 18133-18135
  } while (!v8);                                    // 计数 10 次
  // ---- ⑤ 外部 DLL / 电影 / 对象数组释放后重建 ----
