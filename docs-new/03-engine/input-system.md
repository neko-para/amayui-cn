...
```
⇒ **`get-input-type` = 消息/ADV 的"点击推进"门**。`_this[429812]`（节流间隔）**全工程只在 0xCD 读、从无写入 → bss 初值 0**，故 `_this[429812] <= (now-lastAdvance)` 恒真 → **引擎 get-input-type 实际不节流（每次调用都推进）**；"adv 激活"（`0x8000000`）只是 OR 兜底（恒真）。在此条件下把脚本推进到**鼠标回调注册的跳转目标**（若注册过），否则原样返回。emulator 旧 `advanceThrottle=200` 会引入 ~200ms 输入迟滞，已置 0（对齐引擎无节流）。
