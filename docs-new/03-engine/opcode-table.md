...
| 0x1A0 | 9 |  | sub_42DC70 | 已核对 | **读槽头（292 B，不装载状态）**：op2 = 槽号 → `CreateFileA` RO（打不开 ⇒ `op1 = 1`），`sub_438120` 读头：成功 ⇒ `op1 = 0` + `op3..op8` = 年/月/日/时/分/秒（★头里 `+268` 是星期、本指令**跳过不取**）+ `op9` = 游玩秒数（头 `+280` 的 i32）；头坏 ⇒ `op1 = 2`。handler=sub_42DC70（raw .c 38365-38404）。★emulator 已实现（`handlers/save-slot.ts`） |
