...
- emulator 整跳跳过本脚本（`SLOT_GAPS ⑥`），只等价实现了 BGM 重播（`handlers/save-slot.ts` 的 `replaySavedBgm`）
- 未实现：10 个 SE 通道重装、LOADCHARM/魅力绘制、消息跳过复位、global 14acd9/14acda 一族状态恢复
...
- 备注：读了全文 375 行；未深读 `call-script 51c3`（LOADCHARM）与 `46`（DRAWCHARM）的内部。
