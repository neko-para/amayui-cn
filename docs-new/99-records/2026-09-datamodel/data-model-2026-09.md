...
kind: record
state: consumed
superseded_by: docs-new/00-overview/authority.md
---

> ⏳ **一次性提案（2026-09），已消费**：结论已并入 `docs-new/00-overview/authority.md` 的「附录 · 数据模型」。
> 本文件只作**重构前的诊断与逐份归属依据**保留（§1 的 D1–D9、§7 的逐份归属表、§8 的分期与偏差）。
> 落地时有三处**有意偏差**，见本文件末尾补记。
...
| # | 缺陷 | 证据 |
|---|---|---|
| **D1** | **没有 `kind` / `state` 的机器建模**。真源 / 生成物 / 叙述 / 一次性快照四类只靠正文散文标注（"勿手改"、"一次性"）。 | `docs-new/README.md:71` 至今把 `stub-reaudit-2026-09.md` §4「由代码实算」当卖点，而该表是**过期快照**（`docs-new/99-records/2026-09-audit/raw/audit-final-docs.json:538-542`：声称 OPS 230/50/14，实测 254/51/9）。 |
...
docs-new/
  ...
  99-records/                           ★新  历史区
    ...
    2026-09-live2d/{docs-new/99-records/2026-09-live2d/CONTEXT.md,live2d-deform-semantics.md}
...
| `docs-new/99-records/2026-09-live2d/CONTEXT.md`（仓根） | **移动** → `99-records/2026-09-live2d/docs-new/99-records/2026-09-live2d/CONTEXT.md` + consumed |
...
| **D** | 11 份一次性文档 + `docs-new/99-records/2026-09-live2d/CONTEXT.md` + `live2d-deform-semantics.md` 移入 `99-records/`；改 `evidence[].file` 与 `route-c` 测试路径 | **中高**（断锚风险；需脚本化重写 + `tickets.js --validate`） | `tickets --validate` 绿；`npm run verify` 绿 |
...
## 补记 · 落地时的三处有意偏差（2026-09 执行后追加）

1. **`emulator` 面没有改名为 `impl`**：`emulator.status/evidence/guard/note` 已经是"本工程面"的名字，
   改名要动 4 个代码文件 + 136 条 JSON + 多处散文，收益为零、风险非零 ⇒ **保留 `emulator` 这个名字**，
   只做**面划分**（`engine` / `emulator` / `journal` / `refs`）。
2. **`opcode-gaps.json` 没有并入 `opcodes.json`**：它是**处置台账**（`disposition`+`note`+`ticket`），
   与 `opcodes.json` 的**映射台账**（opcode→handler→语义）是两个实体面 ⇒ 保留为独立文件。
   但 `build-opcode-gaps.mjs` 现在从 `analysis/opcodes.json` 取 name/argc/status（不再依赖 `opcode-table.md`）。
3. **没有把 71/136 条 `note` 的沿革段机械拆进 `journal[]`**：那是 207 段混排散文，自动切分会**丢信息**。
   改为**渲染侧裁剪**（md 只给一句话 + 指向 JSON 的 `--show` 命令），沿革仍在 JSON 里、且**不再重复进 md**——
   这已经满足原始诉求。`journal[]` 字段作为**今后**记录结论级沿革的落点保留（当前为空）。
4. **`opcode-table.md` 内联的 75 KB 审计/订正句未逐条搬走**：那需要逐行人工判断"哪半句是当前语义"，
   574 行规模下属于独立的清理轮。★**但它现在不是真源**（改它会与 `analysis/opcodes.json` 不一致而被守卫打回），
   所以它的沿革不会传染到别处。
