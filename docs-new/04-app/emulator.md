...
- **存档槽**：链路已通（`0x1A0` 读头 / `0x1A1` 读档 / `0x19E` 存档 / `0x1AB` 删 / `0x1AC` 复制 / `0x1AE`·`0x1AF` `.STH`，
  见 [`../03-engine/save-data.md`](../03-engine/save-data.md) §7）。剩余缺口：真槽（format 1..3）的**状态主体**未解析
  （只读头 + 接游玩秒数）、`0x19E` 的覆盖确认框未建模、`.STH` 只做不透明往返（`SLOT_GAPS`）。
