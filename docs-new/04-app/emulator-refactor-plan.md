...
### 1.1 ★拆分的操作纪律（`T-0019` 一次半成品尝试换来的，动手前必读）

上面那张表回答"**拆到哪**"；这一节回答"**怎么拆才不把树留在坏态**"。四条都是实测踩出来的：

1. **先留 barrel、再搬实现**：原文件先变成 barrel 并把 re-export 补齐，**然后**才搬函数。
   反过来做（先搬走再补 barrel）会让中途所有 import 断掉。
2. **每次落盘立刻 `npm run typecheck`**：半成品的语法错会让**所有** `node --import tsx` 停摆
   （测试、报告、调试实例一起）⇒ 一个文件的语法错会**阻塞同波所有单元**，不是"我这块红"。
3. **被切开的 JSDoc 块要在两端各自补回 `/**` 与 `*/`**：实测同一段注释被切在 `handlers/` 与 `vm/`
   两个文件里，两端都成了断注释。
4. **声明归属按 barrel 的 import 走**：判断某个 helper 该留在哪个文件，看**谁 import 它**，
   不要凭名字猜（实测 `defaultWinGeom` 被留在错的文件里）。

> 相关票：`T-0019`（拆分主票，分节边界见 §1）、`T-0020`（测试结构收敛，见 `test-organization.md` §10.1）。
