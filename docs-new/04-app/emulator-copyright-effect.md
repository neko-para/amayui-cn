# Emulator：版权页「frame 效果」实现（架构性调整优先）

> 触发：在改 emulator 前，必须审视当前 **Pixi"按指令推渲染"** 是否匹配引擎**真实渲染策略**。结论：**不匹配**，需 3 点架构性调整。同时**严格 flag 校验**：配置了不认识的 flag 立即中断。
...
⇒ 需要 **3 点架构性调整**（见 §2），且**必须严格拒绝未知 flag**（见 §6）。

---

## 1. 引擎真实渲染模型（现场确认，engine.cpp）
...
### 1.3 门控局部循环 = `effect_flags` 门控级联状态机（"奇怪"所在）
