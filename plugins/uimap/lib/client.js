// @amayui/uimap — Client half (pre-bundled module-loader format).
// Registers the `amayui_uimap` tool card (map selection + clean-workbench entry),
// the full-screen `shell.overlay` modal (map selection view + clean-workbench
// view), and the session-header 「🔍 UI 地图」 one-click button. Client→Host
// communication goes over the `/dsh-uimap/api/*` HTTP endpoints (static bundle
// has no `host.call`), so every interaction survives reload/replay on the host
// side via the shared scan state.
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ---- client→host RPC over the static-bundle HTTP API ----
    async function hostRpc(method, args) {
      let res
      try {
        res = await fetch('/dsh-uimap/api/' + method, {
          method: 'POST',
    ...
    function AmayuiUimapToolView(props) {
      ...
      function doExport() {
        ...
        setExporting(true)
        setExportMsg('')
        hostRpc('uimap-export', { indices: selList }).then((r) => {
          setExporting(false)
    ...
    function UimapOverlay() {
      ...
      function rescan() {
        ...
        setScanBusy(true)
        setErr(null)
        hostRpc('uimap-scan', { png: scanPng || undefined }).then((s) => {
          setScanBusy(false)
          if (s && s.ok && s.scan) {
      ...
      function doExport() {
        ...
        setExporting(true)
        setExportMsg('')
        hostRpc('uimap-export', { indices }).then((r) => {
          setExporting(false)
          if (r && r.ok) setExportMsg('已导出 ' + r.selected_count + ' 个块 → ' + r.path)
          else setExportMsg('导出失败：' + String(r && r.error || '未知错误'))
        }).catch((e) => {
    ...
    // ---- 清理工作台：块清单独立载入 + 组/块下拉；独立清理预览 + 窄拖拽把手；预览刷新/关插值/整像素；地图选中优先 ----
    function CleanWorkbench({ scan, selected }) {
      ...
      function loadFromFile() {
        hostRpc('uimap-clean-list', {}).then((r) => {
      ...
      function doExportPlan() {
        ...
        setExporting(true)
        setPlanMsg('')
        hostRpc('uimap-clean-export', { blocks: plan }).then((r) => {
          setExporting(false)
